...
main(int argc, char **argv)
{
    ...
    strftime(logfilename,128,"project_%Y_%m_%d_%H_%M_%S.log", nowtm);
    dht_log = fopen(logfilename, "w+");
    if (dht_log){
        printf("log in %s\n", logfilename);
    }
    ...
    for(i = 0; i < num_bootstrap_nodes; i++) {
        ...
        char mybuf[200];
        printIP((struct sockaddr*)&bootstrap_nodes[i], mybuf);
        printf("ping node: %s \n", mybuf);
    ...
    while(1) {
        ...
        if(rc > 0) {
            ...
            /*                   &tosleep, callback, NULL);//parse the message with own function */
            rc = check_response(buf, rc, (struct sockaddr*)&from, fromlen,
                                &tosleep, callback, NULL);//parse the message with own function
        } else {
            rc = check_response(buf, rc, (struct sockaddr*)&from, fromlen,
                                &tosleep, callback, NULL);//parse the message with own function
